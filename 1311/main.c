...
int main() {
    ...
    // =====================================
    // Escolha do caminho da imagem
    // =====================================
    char caminhoImagem[256];
    printf("\nDigite o caminho completo da imagem (.mif):\n");
    printf("→ ");
    fgets(caminhoImagem, sizeof(caminhoImagem), stdin);
    caminhoImagem[strcspn(caminhoImagem, "\n")] = '\0'; // remove o \n

    // Se o usuário não digitar nada, não há caminho
    if (strlen(caminhoImagem) == 0) {
        fprintf(stderr, "❌ Caminho da imagem não informado. Encerrando.\n");
        return 1;
    }

    // Verifica se o arquivo existe antes de tentar carregar
    if (access(caminhoImagem, F_OK) != 0) {
    ...
    // =====================================
    int bytes = carregarImagemMIF(caminhoImagem);
    if (bytes < 0) {
    ...
    // ==================================
    // Loop principal de controle de zoom
    // ==================================
    while (1) {
        printf("\nDigite comando (+, -, 0): ");
